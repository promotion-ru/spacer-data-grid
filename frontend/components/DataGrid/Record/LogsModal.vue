<template>
  ...
  >
    <div class="record-logs-modal">
      <!-- Информация о записи -->
      <div v-if="record" class="mb-4 p-4 rounded-lg" style="background: var(--surface-100); border: 1px solid var(--surface-200);">
        <div class="flex items-center space-x-3">
          <i class="pi pi-file-edit text-xl" style="color: var(--primary-500);"></i>
          <div>
            <h3 class="font-semibold" style="color: var(--text-primary);">{{ record.name || 'Без названия' }}</h3>
            <p v-if="record.description" class="text-sm" style="color: var(--text-secondary);">{{ record.description }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="mb-4 space-y-3">
        ...
        <div v-show="showFilters" class="filters-container">
          <div class="rounded-lg p-4 space-y-3" style="background: var(--surface-100); border: 1px solid var(--surface-200);">
            ...
            <!-- Быстрые фильтры -->
            <div class="pt-4" style="border-top: 1px solid var(--surface-200);">
        ...
        <div v-if="pagination" class="text-sm flex items-center justify-between" style="color: var(--text-secondary);">
          <span>
            ...
            <span v-if="hasActiveFilters" class="ml-2" style="color: var(--primary-500);">
              (применены фильтры)
            </span>
          </span>
          <Button
      ...
      <div v-else-if="logs?.length" class="space-y-4">
        ...
        <div class="max-h-96 overflow-y-auto space-y-4">
          <div
            ...
            style="border: 1px solid var(--surface-200); background: var(--surface-0);"
            @mouseenter="$event.target.style.background = 'var(--surface-50)'"
            @mouseleave="$event.target.style.background = 'var(--surface-0)'"
          >
            <!-- Заголовок лога -->
            ...
            <div v-if="log.changes?.length" class="space-y-2">
              ...
              <div class="rounded-md p-3 space-y-2" style="background: var(--surface-50);">
                <div
                ...
                >
                  <span
                    class="font-medium"
                    :style="selectedFieldFilter && change.field.toLowerCase().includes(getFieldKeyByValue(selectedFieldFilter)?.toLowerCase()) ? 'color: var(--primary-500);' : 'color: var(--text-primary);'"
                  >
                    {{ change.field }}:
                  </span>
                  <div class="flex items-center space-x-2 mt-1 sm:mt-0">
                    <span :title="change.old_value"
                          class="px-2 py-1 rounded text-xs max-w-32 truncate"
            ...
            <div v-if="log.metadata?.files_names" class="mt-3">
              <h4 class="text-sm font-medium mb-2" style="color: var(--text-primary);">Файлы:</h4>
              <div class="flex flex-wrap gap-2">
                <Tag
            ...
            <div v-if="log.metadata && Object.keys(log.metadata).length > 0 && !log.metadata.files_names" class="mt-3">
              <details class="text-sm">
                ...
                <div class="mt-2 rounded p-2 text-xs" style="background: var(--surface-50);">
      ...
      <div v-else class="text-center py-8">
        <i class="pi pi-history text-4xl mb-3" style="color: var(--surface-300);"></i>
